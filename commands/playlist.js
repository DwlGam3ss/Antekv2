...
module.exports = {
  ...
  cooldown: 3,
  aliases: ["pl"],
  description: "Odtwórz playliste z YouTube",
  async run(message, args) {
    ...
    if (!args.length)
      return message
        .reply(`Użycie: ${message.client.prefix}playlist <Link playlisty | Nazwa playlisty>`)
        .catch(console.error);
    if (!channel) return message.reply("Musisz najpierw dołączyć do kanału głosowego!").catch(console.error);
    ...
    if (urlValid) {
      try {
        ...
        console.error(error);
        return message.reply("Nie znalazłem takiej playlisty :(").catch(console.error);
      }
    } else {
      try {
    ...
    if (!PRUNING) {
      ...
      if (playlistEmbed.description.length >= 2048)
        playlistEmbed.description =
          playlistEmbed.description.substr(0, 2007) + "\nLista odtwarzania przekracza limit...";
    }

    message.channel.send(`${message.author} Uruchomił playlistę`, playlistEmbed);
